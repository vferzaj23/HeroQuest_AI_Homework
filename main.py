...
# -*- coding: utf-8 -*-
"""
Fantasy Board Search Project: UCS and A*

Domain: Grid-based fantasy board game with energy, monsters, potions, key-door constraint.

This file provides:
- Board and State modeling
- SearchProblem abstraction
- Uniform Cost Search (UCS)
- A* (with an admissible Manhattan heuristic)
- Instrumentation for algorithm comparison on a sample board

Author: Vjosi Ferzaj
Course: CEN352 - Assignment 1
"""
...
class Tile:
    """Enumeration-like values for board tiles."""
    START = "S"
    GOAL = "G"
    EMPTY = "."
    WALL = "W"
...
class Board:
    """
    Represents the fantasy board as a 2D grid of tiles.
    Provides utility methods to query tiles and valid moves.
    """
    ...
    def is_passable(self, pos: Tuple[int, int], has_key: bool) -> bool:
        """
        Door requires key. Walls are impassable. Monsters are passable with extra cost.
...
class State:
    """
    Search state containing:
    - position: (row, col)
    - energy: remaining energy (>= 0 is valid; actions may reduce/increase)
    - has_key: whether the key has been collected
    """
...
class SearchProblem:
    """
    Defines the search problem on the Board:
    - initial state (from S)
    - goal test (reach G with energy >= 0)
    - actions/transition model:
        Move into neighbor:
          cost base + tile-specific costs
          update energy accordingly
          pick up items (key/potion)
    """
    ...
    def successors(self, state: State) -> List[Tuple[State, int, str]]:
        """
        Generate successor states for moves in 4 directions.
        Returns a list of (next_state, step_cost, action_label).
        Action label is for tracing solution paths.
        """
        succs = []
        for nbr in self.board.neighbors4(state.pos):
            ...
            # Compute step cost and energy change
            ...
            if tile == Tile.DOOR:
                # Door consumes unlock cost, only if has_key (passable checked above)
                step_cost += self.cost_unlock
                energy_change -= self.cost_unlock

            # Apply potion/key effects
            next_has_key = state.has_key or (tile == Tile.KEY)
            next_energy = state.energy + energy_change
            if tile == Tile.POTION:
                next_energy += self.potion_energy_gain  # potions restore energy

            # Ignore transitions that drop energy below zero immediately—optional choice.
            # We allow them so UCS/A* can consider paths that momentarily go negative,
            # but goal requires energy >= 0 at G. Often better to prune here:
            if next_energy < 0:
                # Prune unviable moves
                continue
...
class SearchResult:
    """Holds the solution path and metrics."""
    def __init__(
        self,
        found: bool,
        ...
        self.found = found
        self.path = path  # list of (state, action_label) from start to goal
...
def reconstruct_path(came_from: Dict[State, Tuple[Optional[State], int, str]], goal_state: State) -> Tuple[List[Tuple[State, str]], int]:
    ...
    came_from[state] = (parent_state, step_cost, action_label)
    Returns (path list of (state, action_label), total_cost).
    """
    path: List[Tuple[State, str]] = []
    total_cost = 0
    cur = goal_state
...
def uniform_cost_search(problem: SearchProblem) -> SearchResult:
    """
    UCS using a priority queue keyed by path cost g(n).
    """
    start_time = time.perf_counter()

    start = problem.initial_state
    frontier = []  # min-heap of (g_cost, tie_breaker, state)
    tie = 0
...
def admissible_heuristic(state: State, problem: SearchProblem) -> int:
    ...
    - Does not overestimate actual path cost under positive step costs.
    - Simple, domain-agnostic. You can augment conservatively (e.g., +0) if key not in hand.
    """
...
def a_star_search(problem: SearchProblem, heuristic_fn=admissible_heuristic) -> SearchResult:
    """
    A* search using f(n) = g(n) + h(n), where h is admissible.
    """
    start_time = time.perf_counter()

    start = problem.initial_state
    frontier = []  # min-heap of (f_cost, tie_breaker, state)
    tie = 0
...
def main():
    ...
    problem = SearchProblem(
        board=board,
        initial_energy=10,  # tune as desired
        cost_move=1,
        cost_monster=3,
        cost_unlock=2,
